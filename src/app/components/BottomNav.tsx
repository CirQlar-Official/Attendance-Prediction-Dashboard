...
export function BottomNav({ isAdmin, onLeaveGroup, onAuthChange, selectedGroup }: BottomNavProps) {
  ...
  if (isDesktop) {
    return (
      <>
        <div
          className="fixed left-0 top-0 z-50 flex h-full flex-col gap-3 overflow-hidden border-r border-slate-200/80 bg-white/80 px-3 py-5 backdrop-blur-xl dark:border-slate-800/80 dark:bg-slate-900/80"
        ...
        >
          <div className="mb-2 flex min-w-0 items-center px-2" style={{ justifyContent: sidebarOpen ? 'space-between' : 'center' }}>
            ...
            <button
              ...
              className="shrink-0 rounded-xl p-2 text-slate-500 transition hover:bg-slate-100 hover:text-slate-900 dark:text-slate-400 dark:hover:bg-slate-800 dark:hover:text-white"
          ...
          <div className="relative flex flex-1 flex-col gap-1">
            {activeIndex >= 0 && pillStyle.size > 0 && (
              <div className="absolute inset-x-0 rounded-2xl bg-slate-100/90 dark:bg-slate-800/80" style={pillTransitionStyle} />
            )}
            ...
            {tabs.map(({ path, icon: Icon, label, strokeWidth }, i) => (
              ...
              >
                <Icon
                  className={`size-5 shrink-0 ${isActive(path) ? 'text-blue-600 dark:text-cyan-300' : 'text-slate-500 dark:text-slate-400'}`}
                  strokeWidth={strokeWidth}
                />
                <span
                  className={`overflow-hidden text-sm font-medium whitespace-nowrap ${isActive(path) ? 'text-slate-900 dark:text-cyan-300' : 'text-slate-500 dark:text-slate-400'}`}
                  style={{ opacity: sidebarOpen ? 1 : 0, maxWidth: sidebarOpen ? '200px' : '0px', transition: 'opacity 200ms ease-out, max-width 300ms ease-out' }}
                >
                  {label}
                </span>
              </Link>
          ...
          <button
            ...
            className={`flex items-center gap-3 rounded-2xl px-3 py-3 text-slate-500 transition hover:bg-slate-100 hover:text-rose-600 dark:text-slate-400 dark:hover:bg-slate-800 dark:hover:text-rose-300 ${leavingGroup ? 'cursor-not-allowed opacity-50' : ''}`}
  ...
  return (
    <div className="fixed inset-x-0 bottom-0 z-50 border-t border-slate-200/80 bg-white/85 backdrop-blur-xl dark:border-slate-800/80 dark:bg-slate-900/85">
      <div className="mx-auto flex h-[74px] max-w-[560px] items-center overflow-x-auto overflow-y-hidden px-2 py-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        <div className="relative flex h-full min-w-max items-center gap-1">
          {activeIndex >= 0 && pillStyle.size > 0 && (
            <div className="absolute top-2 bottom-2 rounded-2xl bg-slate-100/90 dark:bg-slate-800/80" style={pillTransitionStyle} />
          )}
          ...
          {tabs.map(({ path, icon: Icon, label, strokeWidth }) => (
            <Link key={path} to={path} ref={el => { tabRefs.current[tabs.findIndex(tab => tab.path === path)] = el; }} className="relative z-10 flex h-full shrink-0 flex-col items-center justify-center gap-[4px] min-w-[72px] rounded-2xl px-3 py-2">
              <Icon
                className={`size-5 ${isActive(path) ? 'text-blue-600 dark:text-cyan-300' : 'text-slate-500 dark:text-slate-400'}`}
                strokeWidth={strokeWidth}
              />
              <span className={`text-[12px] font-medium whitespace-nowrap ${isActive(path) ? 'text-slate-900 dark:text-cyan-300' : 'text-slate-500 dark:text-slate-400'}`}>
                {label}
              </span>
            </Link>
          ))}
          ...
          <button
            onClick={handleLeaveGroup}
            disabled={leavingGroup}
            className={`relative z-10 flex h-full shrink-0 flex-col items-center justify-center gap-[4px] min-w-[72px] rounded-2xl px-3 py-2 text-slate-500 transition hover:text-rose-600 dark:text-slate-400 dark:hover:text-rose-300 ${leavingGroup ? 'cursor-not-allowed opacity-50' : ''}`}
            aria-label="Leave group"
          >
            <LogOut size={18} strokeWidth={1.8} />
